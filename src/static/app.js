    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? `<ul class="participant-list">${details.participants.map((participant) => `<li class="participant-entry"><span class="participant-name">${participant}</span><button type="button" class="remove-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}" title="Unregister participant">×</button></li>`).join("")}</ul>`
